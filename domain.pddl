...
(:durative-action bakeMeat
    ...
    :effect(and(at start(and(is_busy ?c)(is_busy ?meat_heater)))
                ...
                (increase(cooked_meat_left ?meat_plate)2)(is_not_busy ?c)))))
...
(:durative-action move
    ...
    (at end(is_not_busy ?c)(at ?c ?to)(not(at ?c ?from)))
    )
...
(:durative-action makeDrink
    ...
    :condition(and (at start(and((is_not_busy ?a)(not_busy ?c))))
            (over all(and (is_combination_point ?a)(not(drink_order_complete ?order))(>(drink_order_Left ?order)0)(at ?c ?a)))
)
